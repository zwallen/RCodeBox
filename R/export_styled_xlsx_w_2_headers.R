...
export_styled_xlsx_w_2_headers <- function(data, ws_title, filename) {
  ...
  openxlsx::setColWidths(wb, ws_title, cols = 1:length(col_widths), widths = col_widths)
  ...
  border_bottom <- openxlsx::createStyle(border = "bottom", borderStyle = "medium")
  border_thin_top <- openxlsx::createStyle(border = "top", borderStyle = "thin")
  openxlsx::addStyle(wb, ws_title, align_left, rows = 1:nrow(data), cols = 1:ncol(data), gridExpand = TRUE)
  openxlsx::addStyle(wb, ws_title, align_center, rows = 1:nrow(data), cols = 1, gridExpand = TRUE)
  openxlsx::addStyle(wb, ws_title, bold, rows = 1:2, cols = 1:ncol(data), gridExpand = TRUE)
  openxlsx::addStyle(wb, ws_title, border_top, rows = 1, cols = 1:ncol(data), gridExpand = TRUE, stack = TRUE)
  openxlsx::addStyle(wb, ws_title, border_bottom, rows = 2, cols = 1:ncol(data), gridExpand = TRUE, stack = TRUE)
  openxlsx::addStyle(wb, ws_title, border_bottom, rows = nrow(data), cols = 1:ncol(data), gridExpand = TRUE, stack = TRUE)
  ...
  for (row in border_rows) {
    openxlsx::addStyle(wb, ws_title, border_thin_top, rows = row, cols = 1:ncol(data), gridExpand = TRUE, stack = TRUE)
  }
  for (col in seq_len(ncol(data))) {
    for (row in 3:nrow(data)) {
      ...
      if (is.character(val) || is.factor(val)) {
        ...
        if (!is.na(num)) {
          openxlsx::writeData(wb, ws_title, num, startCol = col, startRow = row, colNames = FALSE, rowNames = FALSE)
        }
      }
    }
  }
