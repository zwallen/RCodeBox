...
::: {.callout-note}
The example data download and preparation process can be lengthy, so feel free to skip to **@sec-usage-examples** and pickup with the prepped data RDS object.
:::
...
::: {.callout-note}
This has already been done previously and data has been stored in the `GDCdata` directory found in the same directory as this vignette.
:::
...
# Output data to r objects for later
saveRDS(clinical_data, "example_data/clinical_data.rds")
saveRDS(gex_data, "example_data/gex_data.rds")
saveRDS(gex_counts, "example_data/gex_counts.rds")
saveRDS(gex_tpm, "example_data/gex_tpm.rds")
...
The prepared data from this section can be found in the `gdc_data.rds` object in this directory.
...
Lets load the previously prepped TCGA data.

```{r}
#| label: load-data

# Load previously prepped data
clinical_data = readRDS("example_data/clinical_data.rds")
gex_data = readRDS("example_data/gex_data.rds")
gex_counts = readRDS("example_data/gex_counts.rds")
gex_tpm = readRDS("example_data/gex_tpm.rds")
```
