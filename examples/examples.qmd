...
::: {.callout-note}
The example data download and preparation process can be lengthy, so feel free to skip to **@sec-usage-examples** and pickup with the prepped data that's stored in and can be loaded from the package itself.
:::
...
As stated above, the prepared data from this section is contained within the `RCodeBox` package and can be accessed via the `data()` function.
...
clinical_data = data("example_data/clinical_data.rds")
