...
for (id in ensgid) {
  ...
  test_data[[gex_data[id, "gene_name"]]] = factor(
    dplyr::case_when(
      ...
      gsub("-", ".", rownames(test_data)) %in%
        colnames(gex_tpm)[gex_tpm[id, ] < quantiles[2]] ~ "Low",
      .default = NA
    ),
    levels = c("Low", "Moderate", "High", "Very High")
  )
...
g = volcano_plot(
  ...
  figwidth = 2000,
  figheight = 1500,
  out_path = "example_output/figureN.jpg"
)
```

![Differential gene expression analysis results between colon and rectal adenocarcinoma](example_output/figureN.jpg){#fig-N}
